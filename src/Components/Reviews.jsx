...
const Reviews = () => {
  ...
  if (isLoading) {
    return (
      <div className={`flex items-center justify-center min-h-screen ${theme === 'dark' ? 'bg-gray-950 text-gray-200' : 'bg-gray-100 text-gray-800'}`}>
        <p className={`text-xl ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>Loading reviews...</p>
      </div>
    );
  }
  ...
  const reviews = foods.filter(food => food.additionalNotes && food.additionalNotes !== "Enter notes");

  return (
    <div className={` p-8 transition-colors duration-500 max-w-[1700px] w-full rounded-2xl
      ${
        theme === 'dark'
          ...
          reviews.map((food) => (
            ...
            >
              <div className="flex items-center mb-4">
                <img
                  src={food.photoURL || "https://i.ibb.co/6Pq7pXw/default-avatar.png"}
                  alt={food.Name}
